...
public class PlayerStatistics {
	protected static final int WINDOW = 5;

	public static BiPredicate<Player, Game> playedGame = (player, game) -> {
		return player.getTeams().stream()
				.filter(team -> team.isCurrent(game.getDate()))
				.findAny()
				.map(PlayerTeam::getTeam)
				.map(Team::getId)
				.map(teamId -> teamId.equals(game.getHomeTeamId()) || teamId.equals(game.getVisitorTeamId()))
				.orElse(false);
	};
	...
	public static BiFunction<PlayerGame, PlayerTeam, Integer> individualScore = (game, team) -> {
		...
		case atakująca:
		case atakujący:
			return (game.isPrimary() ? 2 : 1) + game.getPoints() + 2 * game.getAces() + 2 * game.getBlocks();
		case libero:
			return 1 + receptionScore.apply(game, team);
		case przyjmująca:
		case przyjmujący:
			return (game.isPrimary() ? 2 : 1) + game.getPoints() + 2 * game.getAces() + 2 * game.getBlocks() + receptionScore.apply(game, team);
		case rozgrywająca:
		case rozgrywający:
			return (game.isPrimary() ? 2 : 1) + 3 * game.getPoints() + 3 * game.getAces() + 2 * game.getBlocks();
		case środkowa:
		case środkowy:
			return (game.isPrimary() ? 2 : 1) + (int) (1.5 * game.getPoints() + 0.5) + 2 * game.getAces() + 2 * game.getBlocks();
		default:
			return 0;
		}
	};
	...
	private final ExponentialMovingAverage pointsPerSetHome = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage playerPointsHome = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage teamPointsHome = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage gamePointsHome = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage pointsPerSetAway = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage playerPointsAway = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage teamPointsAway = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private final ExponentialMovingAverage gamePointsAway = new ExponentialMovingAverage((WINDOW + 1) / 2);
	private int totalPoints = 0;

	public PlayerStatistics(Player player, List<Game> allGames) {
		this.player = player;
		allGames.stream()
			.filter(game -> playedGame.test(player, game))
			.map(Game::getId)
			.map(gameId -> player.getGames().get(gameId))
			.forEach(stats -> {
				if (stats != null) {
					inTeam.append(100);
